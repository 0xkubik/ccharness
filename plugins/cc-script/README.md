...
ccscriptctl roadmap add feat <text>    # a feature → "## Features" section (build-large)
ccscriptctl roadmap add todo <text>    # a medium change → "## TODO" section (build-medium)
ccscriptctl roadmap add fix <text>     # a small fix → "## Fixes" section (build-small)
ccscriptctl roadmap view [what]        # print the roadmap (all | feat|todo|fix)
...
independently. It creates a missing section in its canonical slot (Features → TODO → Fixes) and
writes into the same `## Features` spine that `/roadmap-management` manages. The three sections are
graded by build tier — `## Features` → `build-large`, `## TODO` → `build-medium`, `## Fixes` →
`build-small` — so the section an item lands in is its size tier; `/roadmap-management` reconciles them
on a revision.

`roadmap view` prints straight to the console — `view all` (the default) dumps the whole file, and
`view feat|todo|fix` prints just that one section.
...
- `bin/ccscriptctl` — small terminal helper: opens the project's roadmap / cheat-sheet in your editor (`ccscriptctl roadmap open` · `ccscriptctl cheatsheet`), captures quick roadmap notes (`ccscriptctl roadmap add feat|todo|fix <text>`), prints the roadmap (`ccscriptctl roadmap view [all|feat|todo|fix]`), ticks or removes items by number (`ccscriptctl roadmap done|drop <kind> <n>`), and tidies it (`ccscriptctl roadmap renumber` · `ccscriptctl roadmap prune`); also opens the architect's LikeC4 model (`ccscriptctl architecture open`) and prints the `docs/ccharness/architecture/` tree (`ccscriptctl architecture list`).
