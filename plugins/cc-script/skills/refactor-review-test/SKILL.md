...
- **Print the full folder tree — always** with **`cctreectl`** (a cc-instruments helper that auto-picks the
...
- **The one thing you never decide silently — a behavior/product fork.** When the code's
  ...
  **not** take it to a human and do **not** stop. **Surface it to the conductor (the musician)** —
  ...
  `cc-script:how-to-do`, exactly like `/do` routes one.)* Running standalone with no conductor above
...
until green. **3 strikes on one problem → `cc-instruments:slap`, then pick the fresh approach yourself.**
