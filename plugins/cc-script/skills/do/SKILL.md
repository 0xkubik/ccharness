...
- **A technical fork** (architecture, approach, or build-scope — materially different options, no
  obvious winner) → hand the task to **`cc-script:how-to-do`**, the script's decision loop. Resolving
...
- **Build the full folder tree — always.** Run **`cctreectl`** (a cc-instruments helper — it auto-picks the
  ...
  conventions. If `cctreectl` isn't on PATH (cc-instruments not installed), fall back to `tree`, or
...
**`cc-script:refactor-review-test`** on the change you just built; it owns the rest — the full
...
hand it **up to `cc-script:how-to-do`**; it's the decision loop (not the user), it rules the fork,
...
`cc-instruments:slap` skill**, handing it that problem's context (what you're trying to do, what you
