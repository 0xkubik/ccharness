...
half-done. It is the foot of the build script (what-to-do → how-to-do → do → refactor-review-test)
...
→ STOP and route to `/planner`** (cc-tools builds from the goal outward; don't run ungrounded).
Arriving from how-to-do or under the worker, the North Star already exists — the gate just passes.
...
or which fork is unresolved, then route by *what* is wrong — this is the script talking back
upward, not a dead end:

- **A technical fork** (architecture, approach, or build-scope — materially different options, no
  obvious winner) → hand the task to **`cc-pipeline:how-to-do`**, the script's decision loop. Resolving
...
- **Build the full folder tree — always.** Run **`cctreectl`** (a cc-tools helper — it auto-picks the
  ...
  conventions. If `cctreectl` isn't on PATH (cc-tools not installed), fall back to `tree`, or
...
**`cc-pipeline:refactor-review-test`** on the change you just built; it owns the rest — the full
...
Under the worker this hand-off is transparent: the `do → refactor-review-test` chain runs inside
...
Grounding — no `## Product North Star` → **route to `/planner`**, stop · `0` Gate — technical fork → how-to-do, business → refuse,
vague → brainstorming · `1` Map the codebase — **codegraph if indexed, else grep; always print the
full folder tree via `cctreectl`** · `2` Scope — place the change cleanly (small structural moves OK, tied to this
