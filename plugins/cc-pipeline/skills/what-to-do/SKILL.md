...
The script:
planner grounds the goal → what-to-do fans out a menu (nothing chosen) → how-to-do decides _how_ (one way) → do builds one task and smoke-checks it → refactor-review-test hardens it (refactor · review · full tests) and commits.
...
parenthetical owner may read `what-to-do` or `planner`, both count.

| State       | Path                                                                                                                                                                                                                                                                      |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Absent**  | **Not grounded — route to `/planner` and stop.** Say _"No North Star yet — run `/planner` to set the goal, then re-issue this."_ If a prompt was given, tell the human it didn't run and to re-issue `/what-to-do <theme>` after — **never silently discard the prompt.** |
...
what-to-do runs unbiased — you may nudge _("no feature list yet — `/planner` charts the route
...
- _Under the worker:_ **auto-mark it** `[x]` (no human mid-loop to confirm). See the worker's
  contract.
...
How the human chooses depends on whether a worker loop is driving you:

**Under the worker — do NOT interact.** The worker in open mode runs what-to-do to find its own
direction and must never be blocked on a human. If a worker loop is live
(`.claude/ccharness/worker/state.json` exists with `active:true` and `session_id` == this
...
Phase-3 rank. Hand the **whole list** to `cc-pipeline:how-to-do` — **not one at a time**: how-to-do
...
`0` Ground — `## Product North Star` at the top of `docs/ccharness/roadmap.md`? no → **route to `/planner`**, stop; yes → North Star = goal, route = the `## Features` section (**frontier** = the
...
under the worker: auto-mark) · `2` Fan-out — four lenses (ADD / FINISH / REBUILD / REFACTOR), parallel,
...
multiSelect per lens → checked **list** → how-to-do; under the worker: emit menu as data, **no
...
approach is a bug — that's how-to-do. The human marks _what_; never block a live worker loop.
