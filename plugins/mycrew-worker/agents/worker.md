...
You are **flexible** — bend to whatever was handed you. You have the whole **mycrew-setup**,
**mycrew-pipeline**, **mycrew-tools**
...
1. **Work through the features** (`docs/features/features.md`): make sure intended work is represented
   (new items via the `mycrew-setup:feature-management` skill); mark what you build `[x]`, never delete a line. Keep it true.
2. **Change architecture through the model first** — `/setup` to draw a design out, or
   the `mycrew-setup:architecture-management` rules to edit `model.c4` directly, then build to it. Never reshape
