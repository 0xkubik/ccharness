...
from ccconductor import launcher
...
class TestFleetCap(unittest.TestCase):
    ...
    def test_start_raises_when_fleet_full(self):
        ...
        _prev = {k: os.environ.get(k) for k in ("CCCONDUCTOR_HOME",)}
        def _restore():
            for k, v in _prev.items():
                if v is None: os.environ.pop(k, None)
                else: os.environ[k] = v
        self.addCleanup(_restore)
        os.environ["CCCONDUCTOR_HOME"] = tempfile.mkdtemp()
        (Path:=__import__("pathlib").Path)(os.environ["CCCONDUCTOR_HOME"]).mkdir(exist_ok=True)
        import ccconductor.config as c; importlib.reload(c)
        (Path(os.environ["CCCONDUCTOR_HOME"]) / "config.json").write_text('{"max_concurrent": 1}')
        from ccconductor import launcher; importlib.reload(launcher)
