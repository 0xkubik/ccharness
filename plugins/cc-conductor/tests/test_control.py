...
class TestControl(unittest.TestCase):
    def setUp(self):
        ...
        _prev = {k: os.environ.get(k) for k in ("CCCONDUCTOR_HOME", "CLAUDE_CONFIG_DIR")}
        def _restore():
        ...
        import importlib, ccconductor.config, ccconductor.paths, ccconductor.registry, ccconductor.control
        for m in (ccconductor.config, ccconductor.paths, ccconductor.registry, ccconductor.control):
            importlib.reload(m)
        self.control = ccconductor.control
    ...
    def test_resolve_prefers_meta_pid(self):
        ...
        self.assertTrue(info["launched_by_ccconductorctl"])
