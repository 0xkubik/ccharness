    ...
    for e in entries:
        ...
        if ident == sid or (aid and ident == aid) or (sid and sid.startswith(ident)):
            ...
            return {
                ...
                "launched_by_ccconductorctl": e.get("launched_by_ccconductorctl", False),
            }
    return None
...
def steer_agent(info, message, *, sender=os.killpg, spawn=None):
    if info.get("is_musician"):
        return ("refused-musician", "redirect a musician via its own script / /musician-cancel, not steer")
