#!/usr/bin/env bash
# cc-conductor usage bridge — a statusLine wrapper that captures Claude Code's usage limits.
#
...
# cc-musician musician in ANY project read it to gate expensive work on remaining headroom.
#
...
try:
    ...
    body = {
        ...
        "source": "cc-conductor/usage-statusline",
    }
...
' 2>/dev/null
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    printf '%s' "$PAYLOAD" | jq -c --arg ts "$ts" \
      '{captured_at:$ts, five_hour:.rate_limits.five_hour, seven_day:.rate_limits.seven_day, source:"cc-conductor/usage-statusline"}' \
