...
> **Layer:** cc-instruments is the **helpers layer** of the cc-* harness — philosophy-agnostic tools usable
> in any project, plus the harness's setup wizard and its recommended rules. The product script lives
> in **cc-script** (which depends on this); **cc-musician** (the musician) and **cc-conductor** (the fleet
> orchestrator) sit above.

Plain Markdown + JSON (instructions for Claude Code — no app code, no build):

- **`/crux`** and **`/slap`** — two reasoning side doors that work in any project, with or without the script.
...
The product script (`/roadmap-management` → `/what-to-do` → `/how-to-do` → `/do` → `/refactor-review-test`)
lives in the **cc-script** plugin — install it too if you want the script.
...
| **`/slap`** | The **reset.** When a fix has gone three rounds deep in a rabbit hole, forces a step back: restate the problem, list what was tried, question assumptions, research alternatives, propose a fresh angle. The script's `/do` and `/refactor-review-test` invoke it at three strikes. | "Stop digging — rethink this." |
...
- `skills/slap/SKILL.md` — the reset protocol, invoked by the script's `/do` at three strikes (and by you via `/slap`).
