...
to branch, commit, or push — decide and do it yourself as a normal part of finishing work, and report
...
- **Keep `.gitignore` current.** Owning the history includes owning what's tracked. When build
  ...
  the tree, add them to `.gitignore` instead of committing them or leaving them as noise. Match the
