...
The bridge is a statusLine wrapper: it tees Claude Code's rate_limits payload into
<project>/.claude/ccharness/usage.json and forwards the payload to the user's real
status line. Covers capture, the no-clobber-on-empty rule, downstream passthrough,
...
def usage_path(proj):
    return Path(proj) / ".claude" / "ccharness" / "usage.json"
...
class TestUsageBridge(unittest.TestCase):
    ...
    def test_captures_rate_limits(self):
        ...
        data = json.loads(usage_path(self.proj).read_text())
    ...
    def test_no_downstream_renders_nothing_but_still_writes(self):
        ...
        self.assertTrue(usage_path(self.proj).exists())
