...
re-surfaces every few prompts, reconciles your docs against reality, and offers to run `/find-goal`.
Idempotent — re-run any time (e.g. on a new machine).

The product funnel (`/find-goal` → `/what-to-do` → `/how-to-do` → `/do` → `/refactor-review-test`)
