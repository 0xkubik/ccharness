...
- **A goal is always required, and it must come from `/find-goal`.** Before writing, building, or
  ...
  `.claude/ccharness/roadmap.md` that `/find-goal` writes. If it's missing, surface that and ask the
  user to run `/find-goal` — don't infer a goal from thin context and run with it.
- **A feature list is required once the work is multi-step.** For anything beyond a small,
  self-contained change, the flat, ordered feature list `/find-goal` writes below the North Star in
  `.claude/ccharness/roadmap.md` must exist before execution starts. A one-line fix needs only the
  North Star at the top; a feature needs the list too, so route to `/find-goal`.
