...
# `musicians nonstop on`, removed by `musicians nonstop off`. When armed AND the musician has just
...
# the musician's own brakes and `musicians nonstop off` still work. This hook is DUMB: it only
...
REASON="🔁 nonstop is ARMED and the musician just CLOSED — do NOT stop. Read its outcome in $MUS_STATE. If outcome is \"declined\" (open mode: nothing worth doing — the roadmap frontier is exhausted), DISARM by deleting the marker file $NS_STATE, then stop. Otherwise (achieved / blocked) re-launch the musician for the next piece: invoke /musician with NO prompt (open mode) — it picks the next roadmap milestone itself via what-to-do and its git-notes awareness; you do NOT pick or plan, you only re-invoke. If /musician cannot arm (no North Star), DISARM by deleting $NS_STATE, then stop. To disarm manually: \`musicians nonstop off\`; Esc to interrupt."

if command -v jq >/dev/null 2>&1; then
  jq -n --arg r "$REASON" \
        --arg m "🔁 nonstop -> re-launching the musician for the next piece (open mode; \`musicians nonstop off\` to stop)" \
