...
4. Otherwise build ONE step toward done_when: dispatch a cc-tools:how-to-do subagent → dispatch a cc-tools:do subagent WITH worktree isolation (Agent isolation:"worktree" — the build, its tools, and its sub-agents all stay in one worktree under .claude/worktrees/; a "cd into a worktree" instruction leaks back to main). IT writes the code (you never Edit/Write product code yourself), builds+smoke, chains to refactor-review-test which makes the LOCAL commit INSIDE the worktree (no push). Capture the returned worktreePath + worktreeBranch, then land it via the worktree helper whose absolute path is recorded in state under the worktree_helper key (read it from <run>/state.json): if the build committed, run that helper with [integrate worktreePath worktreeBranch] (INTEGRATED=sha → on your branch + worktree removed; CONFLICT=branch → surface it as a blocker, the worktree is kept); if it produced NO commit, run it with [discard worktreePath worktreeBranch]. how-to-do has NO new buildable approach left (technical path exhausted) → set active:false outcome:"blocked", report the blocked queue, END TURN. Async build that cannot finish in-turn → set awaiting and END TURN (no cycle). Handback by kind: business/non-technical blocker do refuses → set active:false outcome:"blocked", report the blocked queue, END TURN; technical fork / stuck (slap twice) → append to blocked.jsonl and re-run how-to-do for a DIFFERENT approach. External transient block → suspend (awaiting). Append a log line, bump cycle (atomic), END TURN.
...
if [ -n "$ULTRA" ]; then
  REFEED="$REFEED
ULTRACODE (mandatory this run): push parallelism to the max in step 4's build — author a Workflow and/or dispatch parallel cc-tools:do subagents, EACH with its own worktree isolation, and integrate each one's branch (via the recorded worktree helper) as it lands; verify findings adversarially. These tools are always permitted; --ultracode makes them required, not optional."
fi
