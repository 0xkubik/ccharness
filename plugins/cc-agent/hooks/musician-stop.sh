...
5. Otherwise build ONE step toward done_when: cc-tools:how-to-do → cc-tools:do → verify → LOCAL commit (no push). Async build that cannot finish in-turn → set awaiting and END TURN (no cycle/streak). Handback (unbuildable/forked, or slap twice) → append to blocked.jsonl, no-progress cycle. External transient block → suspend (awaiting), do not streak++. Update no_progress_streak (reset on real progress; ++ on blocked/idle), append a log line, bump cycle (atomic), END TURN.
6. ON ANY CLOSE (achieved/declined/gave-up/capped): before END TURN, append ONE past-tense closed-fact line as awareness for the next run — git notes append -m "<built|declined|dead-end>: <what> — <why>". NEVER a forward intent (those go to roadmap-proposals.md, not git notes).
You stop ONLY by flipping active:false (achieved/declined/gave-up/capped), or the user runs /musician-cancel. The brain may DECLINE before building — that is a success, not a give-up. Do not otherwise stop.
PROMPT
)"
