...
goes through a `cc-funnel:do` subagent, because editing inline bypasses `do`'s fork-test,
...
2. tasks EMPTY → DECOMPOSE. Break this piece of work into an ordered list of concrete subtasks, each
          ...
          mode → first dispatch a cc-funnel:what-to-do subagent (menu as DATA, NO AskUserQuestion),
...
3. tasks REMAIN → take the FIRST incomplete one (in_progress, else first pending), mark it
          ...
          • BUILD task → capture BASE = `git rev-parse HEAD`, dispatch a cc-funnel:do subagent with
            ...
            + smoke, chains to cc-funnel:refactor-review-test which makes the LOCAL commit INSIDE the
          ...
          • THINK task (a real fork, a fuzzy pain) → dispatch cc-funnel:how-to-do / cc-tools:crux.
