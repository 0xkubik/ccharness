...
BIN = ROOT / "bin" / "ccmusician"
ALIAS = ROOT / "bin" / "musicians"
...
class TestCcmusician(unittest.TestCase):
    # --- grammar is verb-first (info <id>, nonstop <id> on|off), unified with ccmaestro ---
    ...
    def test_ls_json_is_machine_readable(self):
        # `ls --json` feeds the federated maestro layer — it must emit parseable JSON with the run.
        repo = repo_with_run()
    ...
    def test_task_progress_shown(self):
        # main's task-list progress survives the grow into ccmusician: ls + info show "N/M".
        repo = tempfile.mkdtemp()
    ...
    # --- musicians stays as a thin alias for ccmusician (name absorbed, F4) ---
