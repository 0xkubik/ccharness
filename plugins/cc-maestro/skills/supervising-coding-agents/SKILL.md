...
|---|---|
| `ccmaestro ls [--json]` | All agents: tokens, last activity, verdict, cwd, name (musician rows carry status / task progress / current task / latest action) |
| `ccmaestro musician [<id>] [--json]` | Running musicians in detail: status, what was asked, task progress (`done/total`), the current task, and a tail of the live action feed (what it's doing now). Omit `<id>` to list all |
