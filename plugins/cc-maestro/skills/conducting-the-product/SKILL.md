...
2. READ each module: its roadmap (.claude/ccharness/roadmap.md → North Star + frontier) and its live
   musicians — `"${CLAUDE_PLUGIN_ROOT}/bin/ccmaestro" musician --json` (status, what was asked, task
...
```bash
"${CLAUDE_PLUGIN_ROOT}/bin/ccmaestro" start --repo <module-path> --yolo \
...
Then **observe across passes** with `ccmaestro musician --json` / `ccmaestro ls --json`: a provider
that has closed `achieved` unblocks its consumers; one still `working` / `suspended` you keep holding.

## What you do NOT do

- **You don't build.** No code, no edits — every build is a musician's, through its own funnel.
...
**One pass:** DISCOVER git-repo subfolders → READ each (roadmap + live musicians via `ccmaestro
musician --json`) → THINK (cross-repo dependencies + order) → ASSIGN selectively (`ccmaestro start
