---
description: "Create or maintain the product's roadmap — the North Star goal + ordered feature list in .claude/ccharness/roadmap.md. First run charters it (goal → features → review); a re-run rethinks it freely; given one concrete feature it helps you think it through and records a line (with --force, it formulates and writes after one confirm). Every other cc-tools skill routes here when no North Star exists; what-to-do biases its menu toward the current frontier."
...
**North Star** (vision · core problem · level) at the top of `.claude/ccharness/roadmap.md`, then lays
